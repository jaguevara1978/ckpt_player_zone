...
/**
 * @ngdoc service
 * @name app.CelebrationService
 * @description
 * # UserService
 * Factory in the app.
 */
angular.module('app').factory('SorryService', SorryService);

    function SorryService () {
    
        var width, height, largeHeader, canvas, ctx, triangles, target, animateHeader = true, headerInitiated = false;
        var colors = ['72,35,68', '43,81,102', '66,152,103', '250,178,67', '224,33,48'];
        var service = {};

        service.initHeader = initHeader;
        service.addTriangle = addTriangle;
        service.initAnimation = initAnimation;
        service.tweenTriangle = tweenTriangle;
        service.addListeners = addListeners;
        service.Triangle = Triangle;
        service.stopAnimation = stopAnimation;

        return service;
    
        function initHeader() {
            if (headerInitiated) return;
            headerInitiated = true;
            width = document.getElementById('aside-dialog').offsetWidth;
            height = document.getElementById('aside-dialog').offsetHeight / 2;
